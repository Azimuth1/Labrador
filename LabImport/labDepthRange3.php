...
if($_GET["matrix"]){
	...
	if (($handle = fopen($csv, "r")) !== FALSE) {
		while (($data = fgetcsv($handle, 20000, ",")) !== FALSE) {
			...
			// assume bottom depth = top_depth+0.1
			if(!$bottom_depth)
				$data[5] = $bottom_depth = $top_depth + 0.1;

			if($minDepth && $top_depth < $minDepth)
				continue;

			if($maxDepth && $bottom_depth > $maxDepth)
				continue;
...
/*
function get_col($att)
{
	global $col_headers;
	$found_at = array_search( $att, $col_headers);
	return $found_at;
}
*/
