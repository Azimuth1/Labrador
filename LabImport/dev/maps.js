...
$(document).ready(function () {
  ...
  map = new L.Map('map', {
   ...
   //New CoxcombCharts for labs
    ...
    filterItems = [];
    $('.chemical-filters:checked').each(function(d,e){filterItems.push(e.value)})
...
//savedSettings.x=x
    ...
    function setup_chemical_filter_form() {
        $.getJSON('get_chemical_filters.php', function (filters) {
           ...
           var compounds = _.chain(labdata.features).map(function(d){return _.keys(d.properties.Compounds)}).flatten().unique().sort().value();
           ...
           $.each(compounds, function () {
              ...
              $("#chemical-filter-form").append($("<label>").text(compound_name).prepend(
                 $("<input>").attr('type', 'checkbox').val( compound_name).attr('id', compound_name).attr('class', 'chemical-filters')
              ));
           });
